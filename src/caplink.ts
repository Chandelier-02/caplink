...
const isNativeEndpoint = (x: unknown): x is Worker | MessagePort => {
  return (
    ("Worker" in globalThis && x instanceof globalThis.Worker) ||
    ("MessagePort" in globalThis && x instanceof globalThis.MessagePort)
  );
};
const isNativeConvertible = (
  x: unknown
): x is { [toNative](): MessagePort } => {
...
const proxyTransferHandler = {
  canHandle: (val): val is Proxy | ProxyMarked =>
    proxyMarker in val || createEndpoint in val,
  serialize(obj, ep) {
    let port;
...
export function expose(
  ...
  if (locked.has(ep))
    throw Error(
      "Endpoint is already exposing another object and cannot be reused."
    );
  locked.add(ep);
  ...
  ep.addEventListener(
    "message",
    //@ts-ignore
    async function callback(ev: MessageEvent<unknown>): Promise<void> {
          ...
          case MessageType.SET:
            {
              ...
              parent[data.path.slice(-1)[0]] = fromWireValue.call(
                ep,
                data.value
              );
              returnValue = true;
  ...
  if (isReleased) {
    throw new Error(
      "Proxy has been released and is not useable" +
        (typeof isReleased === "string" ? `: ${isReleased}` : ""),
  ...
  if (endpointState.has(ep)) {
    ...
    try {
      const releasedPromise =
        !force && requestResponseMessage(ep, { type: MessageType.RELEASE });
      ...
      resolvers.forEach(({ reject }) =>
        reject(
          new DOMException("Cancelled due to endpoint release", "AbortError")
        )
      );
      resolvers.clear();
...
export function windowEndpoint(
  ...
  return {
    postMessage: (msg: any, transfer: Transferable[]) =>
      w.postMessage(msg, targetOrigin, transfer),
...
const makeMessageHandler =
  (resolverMap: ResolversMap<MessageId, WireValue>) =>
  (ev: MessageEvent<WireValue | null>) => {
    const { data } = ev;
    if (!data?.id) {
      return;
    }
